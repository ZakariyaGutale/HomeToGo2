...
namespace HomeToGo.Controllers
{
    ...
    public class ReservationController : ControllerBase
    {
        ...
        public async Task<ActionResult<bool>> CheckOverlap([FromBody] Reservation reservation)
        {
            try
            {
                bool isOverlap = await _reservationRepository.IsReservationOverlap(reservation);
                return Ok(isOverlap);
            }
            catch (Exception e)
            {
                _logger.LogError($"Error checking reservation overlap: {e.Message}");
                return StatusCode(500, "Internal server error occurred.");
            }
        }
